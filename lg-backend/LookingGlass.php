...
class LookingGlass
{
    ...
    private static $plainTextMode = false;
    ...
    private static function procExecute(array $cmd, string $host, int $failCount = 2): bool
    {
        ...
        // Выбираем форматирование в зависимости от режима
        $lineEnding = self::$plainTextMode ? PHP_EOL : '<br />';
        $indent = self::$plainTextMode ? '  ' : '&nbsp;';
        ...
        while (($str = fgets($pipes[1], 4096)) != null) {
            ...
            // В plain text режиме не нужно экранировать HTML
            ...
            elseif ($type === 'traceroute') {
                if ($match < 10 && preg_match('/^[0-9] /', $str, $string)) {
                    $str = preg_replace('/^[0-9] /', $indent.$string[0], $str);
                    $match++;
                }
                ...
                if (strpos($str, '* * *') !== false) {
                    ...
                    if ($lastFail !== 'start'
                        ...
                        echo str_pad($str.$lineEnding.'-- Traceroute timed out --'.$lineEnding, 4096, ' ', STR_PAD_RIGHT);
                        break;
                    }
                    $lastFail = $traceCount;
                }
            ...
            echo str_pad($str.$lineEnding, 4096, ' ', STR_PAD_RIGHT);
